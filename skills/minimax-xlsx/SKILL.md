...
```bash
# Git Bash (preferred)
python SKILL_DIR/scripts/xlsx_unpack.py input.xlsx /tmp/xlsx_work/
...
python SKILL_DIR/scripts/xlsx_pack.py /tmp/xlsx_work/ output.xlsx

# PowerShell fallback
python SKILL_DIR/scripts/xlsx_unpack.py input.xlsx $env:TEMP\xlsx_work\
python SKILL_DIR/scripts/xlsx_pack.py $env:TEMP\xlsx_work\ output.xlsx
```
...
# Git Bash (preferred)
python SKILL_DIR/scripts/xlsx_unpack.py input.xlsx /tmp/xlsx_work/
python SKILL_DIR/scripts/xlsx_add_column.py /tmp/xlsx_work/ --col G \
...
python SKILL_DIR/scripts/xlsx_insert_row.py /tmp/xlsx_work/ --at 5 \
    --sheet "Budget FY2025" --text A=Utilities \
    --values B=3000 C=3000 D=3500 E=3500 \
    --formula 'F=SUM(B{row}:E{row})' --copy-style-from 4
python SKILL_DIR/scripts/xlsx_pack.py /tmp/xlsx_work/ output.xlsx
```
**Row lookup rule**: When the task says "after row N (Label)", always find the row by searching for "Label" in the worksheet XML:
- Git Bash: `grep -n "Label" /tmp/xlsx_work/xl/worksheets/sheet*.xml`
- PowerShell fallback: `Select-String -Path "$env:TEMP\xlsx_work\xl\worksheets\sheet*.xml" -Pattern "Label"`
- Or check sharedStrings.xml directly with the Read tool

Use the actual row number + 1 for `--at`. Do NOT call `xlsx_shift_rows.py` separately — `xlsx_insert_row.py` calls it internally.
...
This skill runs on **Windows**. Apply these rules for every shell command:

### Python command
Always use `python` (not `python3`). Windows installs Python as `python`.

### Temp directory
Prefer Git Bash path `/tmp/xlsx_work` (Git Bash maps `/tmp/` to the Windows temp folder automatically).
If Git Bash is unavailable, use PowerShell: `$env:TEMP\xlsx_work`.

### Path quoting (critical on Windows)
Always wrap **all file paths** in double quotes when calling Python scripts. Windows paths
with backslashes are misinterpreted by Git Bash (treating `\` as an escape character) and
paths with spaces fail in both shells without quotes.

```bash
# ✅ Correct — paths quoted
python "C:\Users\you\.claude-internal\skills\minimax-xlsx\scripts\xlsx_reader.py" "G:\project\MyFile.xlsx"

# ❌ Wrong — unquoted backslash paths break in Git Bash
python C:\Users\you\skills\xlsx_reader.py G:\project\MyFile.xlsx
```

If a file is not found, verify the path with `find` (Git Bash) or `Get-ChildItem -Recurse`
(PowerShell) before retrying — don't assume the default location.

### Text search in XML files
**Git Bash (preferred):**
```bash
grep -n "Label" /tmp/xlsx_work/xl/worksheets/sheet*.xml
```
**PowerShell fallback:**
```powershell
Select-String -Path "$env:TEMP\xlsx_work\xl\worksheets\sheet*.xml" -Pattern "Label"
```

### Detecting which shell is available
```bash
# Try Git Bash first — if this succeeds, use bash syntax throughout
bash --version 2>&1
# If not found, fall back to PowerShell for all subsequent commands
```

## Utility Scripts

```bash
python SKILL_DIR/scripts/xlsx_reader.py input.xlsx                 # structure discovery
python SKILL_DIR/scripts/formula_check.py file.xlsx --json         # formula validation
python SKILL_DIR/scripts/formula_check.py file.xlsx --report       # standardized report
python SKILL_DIR/scripts/xlsx_unpack.py in.xlsx /tmp/work/         # unpack for XML editing
python SKILL_DIR/scripts/xlsx_pack.py /tmp/work/ out.xlsx          # repack after editing
python SKILL_DIR/scripts/xlsx_shift_rows.py /tmp/work/ insert 5 1  # shift rows for insertion
python SKILL_DIR/scripts/xlsx_add_column.py /tmp/work/ --col G ... # add column with formulas
python SKILL_DIR/scripts/xlsx_insert_row.py /tmp/work/ --at 6 ...  # insert row with data
```
